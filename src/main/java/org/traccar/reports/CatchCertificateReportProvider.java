...
public class CatchCertificateReportProvider {
    ...
    @Inject
    public CatchCertificateReportProvider(Config config, ReportUtils reportUtils, Storage storage) {
        this.config = config;
        this.reportUtils = reportUtils;
        this.storage = storage;
    }

    public Collection<ElbCatchCertificate> getObjects(long userId, Collection<Long> deviceIds, Collection<Long> groupIds,
    ...
    public void getExcel(OutputStream outputStream,
        ...
        for (Device device: DeviceUtil.getAccessibleDevices(storage, userId, deviceIds, groupIds)) {
            var priorNotifications = PriorNotificationUtil.getCatchCertificate(storage, device.getId(), from, to);
        ...
        File file = Paths.get(config.getString(Keys.TEMPLATES_ROOT),
                "export",
                "catch-certificate-report.xlsx").toFile();
